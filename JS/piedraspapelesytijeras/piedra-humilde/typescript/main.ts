...
const mensajevictoria = document.getElementById('mensajevictoria') as HTMLDivElement;
...
function startgame(opcion: string) {
    if (mispuntos < 3 && puntoscomputer < 3) {
        ...
        if (opcion === opcioncomputer) {
            ...
            (opcion === cartas[0] && opcioncomputer === cartas[2]) ||  // Piedra gana a Tijera
...
document.addEventListener('DOMContentLoaded', () => {
    const piedra = document.getElementById('piedra') as HTMLButtonElement;
    const papel = document.getElementById('papel') as HTMLButtonElement;
    const tijera = document.getElementById('tijera') as HTMLButtonElement;
