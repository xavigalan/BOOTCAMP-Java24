...
CREATE TABLE IF NOT EXISTS `estudiantes_cursos` (
  ...
  CONSTRAINT `fk_estudiantes_cursos_estudiantes`
    FOREIGN KEY (`estudiante_id`)
    REFERENCES `estudiantes` (`estudiante_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_estudiantes_cursos_cursos`
...
INSERT INTO `estudiantes` (nombre_estudiante, primer_apellido, segundo_apellido) VALUES 
...
LEFT JOIN 
    escuela.cursos AS c ON p.profesor_id = c.profesor_id
GROUP BY 
    p.profesor_id, p.nombre_profesor;
...
LEFT JOIN 
    escuela.estudiantes_cursos AS ec ON e.estudiante_id = ec.estudiante_id
GROUP BY 
    e.estudiante_id, e.nombre_estudiante, e.primer_apellido;